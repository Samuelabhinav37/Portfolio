...
(function(){
  ...
  function draw(){if(document.hidden){if(!reduce)requestAnimationFrame(draw);return;}var cw=canvas.clientWidth,chh=canvas.clientHeight;if(cw>10&&canvas.width!==cw){canvas.width=cw;}if(chh>10&&canvas.height!==chh){canvas.height=chh;}var W=canvas.width,H=canvas.height;if(W<=0||H<=0){if(!reduce)requestAnimationFrame(draw);return;}ctx.clearRect(0,0,W,H);rotY+=0.006;var now=performance.now();if(morphT<1){morphT=Math.min(1,(now-morphStart)/MORPH_DUR);var te=ease(morphT);nodes.forEach(function(n,i){var fp=fromPts[i]||[0,0,0],tp=toPts3d[i]||[0,0,0];n.tx=lerp(fp[0],tp[0],te);n.ty=lerp(fp[1],tp[1],te);n.tz=lerp(fp[2],tp[2],te);});}
  ...
  setTimeout(function(){init();draw();if(!reduce){setTimeout(fireConv,400);setTimeout(fireConv,1100);setTimeout(fireConv,1900);setInterval(fireConv,1600);}},200);
})();
