/* About page ASCII-art video converter (middle card), continuous rAF loop. Extracted from an inline <script> block, unmodified. */
/* ASCII filter (middle card) */
(function(){
...
(function(){
  ...
  function draw(){var cw=canvas.clientWidth,chh=canvas.clientHeight;if(cw>10&&canvas.width!==cw){canvas.width=cw;}if(chh>10&&canvas.height!==chh){canvas.height=chh;}var W=canvas.width,H=canvas.height;if(W<=0||H<=0){requestAnimationFrame(draw);return;}ctx.clearRect(0,0,W,H);rotY+=0.006;var now=performance.now();if(morphT<1){morphT=Math.min(1,(now-morphStart)/MORPH_DUR);var te=ease(morphT);nodes.forEach(function(n,i){var fp=fromPts[i]||[0,0,0],tp=toPts3d[i]||[0,0,0];n.tx=lerp(fp[0],tp[0],te);n.ty=lerp(fp[1],tp[1],te);n.tz=lerp(fp[2],tp[2],te);});}
  ...
  packets.forEach(function(p){p.t+=p.speed;var a=proj[p.a],b=proj[p.b];var x=a.sx+(b.sx-a.sx)*p.t,y=a.sy+(b.sy-a.sy)*p.t,s=a.scale+(b.scale-a.scale)*p.t;for(var i=0;i<5;i++){var tp2=Math.max(0,p.t-i*0.025);ctx.beginPath();ctx.arc(a.sx+(b.sx-a.sx)*tp2,a.sy+(b.sy-a.sy)*tp2,1.5*s,0,Math.PI*2);ctx.fillStyle='rgba(255,255,255,'+(0.35*(1-i/5)).toFixed(3)+')';ctx.fill();}ctx.beginPath();ctx.arc(x,y,2.5*s,0,Math.PI*2);ctx.fillStyle='rgba(255,255,255,0.92)';ctx.shadowColor='rgba(255,255,255,0.45)';ctx.shadowBlur=7;ctx.fill();ctx.shadowBlur=0;});
  packets=packets.filter(function(p){return p.t<1;});
  nodes.map(function(n,i){return{n:n,p:proj[i],i:i};}).sort(function(a,b){return a.p.depth-b.p.depth;}).forEach(function(item){var n=item.n,p=item.p,glow=activeNodes[item.i]||0,r=(4+glow*2)*p.scale,da=0.20+(p.depth+1)/2*0.45;if(glow>0.05){ctx.beginPath();ctx.arc(p.sx,p.sy,r+5+glow*4,0,Math.PI*2);ctx.strokeStyle='rgba(255,255,255,'+(0.09*glow).toFixed(3)+')';ctx.lineWidth=1.5;ctx.stroke();}ctx.beginPath();ctx.arc(p.sx,p.sy,r,0,Math.PI*2);ctx.fillStyle='rgba(255,255,255,'+Math.min(0.92,da+glow*0.40).toFixed(3)+')';if(glow>0.1){ctx.shadowColor='rgba(255,255,255,0.40)';ctx.shadowBlur=10;}ctx.fill();ctx.shadowBlur=0;if(p.depth>-0.3){var la=Math.min(0.50,(p.depth+1)/2*0.30+glow*0.38);ctx.font=(5*p.scale+2)+'px JetBrains Mono,monospace';ctx.fillStyle='rgba(255,255,255,'+la.toFixed(3)+')';ctx.textAlign='center';ctx.fillText(n.name,p.sx,p.sy-r-3);}});
  requestAnimationFrame(draw);}
  setTimeout(function(){init();draw();setTimeout(fireConv,400);setTimeout(fireConv,1100);setTimeout(fireConv,1900);setInterval(fireConv,1600);},200);
})();
