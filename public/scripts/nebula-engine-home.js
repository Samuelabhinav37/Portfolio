...
/* ─── Background tuning ───────────────────────────────────────────────
   Two knobs so the atmosphere stays one-number-tunable.
   ATMO_DIM : global multiplier on nebula / starfield / bloom alpha.
              Lower = dimmer atmosphere. 1.0 = original.
   CALM     : multiplier on event respawn intervals (auto-pings, traceroutes,
              intrusions, storms, wormholes, shooting stars). Higher = rarer
              events = calmer. 1.0 = original cadence.                      */
var ATMO_DIM = 0.40;
var CALM     = 1.75;
/* BG_PACE : global multiplier on the SPEED of all background motion (nebula
   drift, node sway, shooting stars, scan rings, pulses, parallax). Lower =
   slower, more deliberate. 1.0 = original. Independent of CALM (frequency).  */
var BG_PACE  = 0.55;

/* CRAFT — ships/satellites are drawn on the same canvas as the dimmed
   nebula/starfield, so pushing ATMO_DIM down would dim them right along
   with everything else. This boosts their aura/hull alpha at the source
   (drawChaseShip, drawSatBody) so they stay hot against a darker sky.
   Live-tunable: window.SITE.CRAFT.boost / .halo in devtools. */
var CRAFT = window.SITE.CRAFT = { boost: 2.05, halo: 1.45 };
...
/* ──────────────────────────────────────────────────────────────────
   ...
   ────────────────────────────────────────────────────────────────── */
(function initGL(){
  /* Phones skip the WebGL nebula shader under LOW_POWER — reuses the same
     no-op shim path as a failed/unavailable WebGL context below. Tablets
     (LOW_POWER true but width>=600) keep it; it's a single fullscreen-quad
     shader, not heavy. */
  ...
  /* Nebula is soft, cached at ~5Hz, and composited as a blitted quad, so it
     does not need HiDPI backing. Capping dpr at 1.0 halves+ the per-frame blit
     fill and the FBO texture bandwidth on HiDPI panels with no visible change
     (the content has no high-frequency detail to lose). */
...
/* Honor prefers-reduced-motion: gate optional simulation systems (grain,
   ...
   LOW_POWER (viewport-width or reduced-motion driven, set early in <head>)
   folds in here too, so the same systems back off on phones/tablets.       */
...
var canvasMXt=CW/2,canvasMYt=CH/2;   // cursor inverse-mapped through the global constellation transform
/* #screen is inside the position:fixed #window, so its rect only moves on
   resize. Cache it instead of reading getBoundingClientRect() on every
   mousemove (which forced a layout flush dozens of times a second). */
var _scrRect=SCR.getBoundingClientRect();
function _refreshScrRect(){ _scrRect=SCR.getBoundingClientRect(); }
addEventListener('resize',_refreshScrRect,{passive:true});
SCR.addEventListener('mousemove',function(e){var r=_scrRect;canvasMX=((e.clientX-r.left)/r.width)*CW;canvasMY=((e.clientY-r.top)/r.height)*CH;});
...
Node.prototype.update = function(dt){
  ...
  if(this.willDie && U.uTime.value > this.dieAt && !this.dead){
    this.dead=true; this.deadA=1.0; _hubsAlive=null;   /* node died: drop cached alive-hubs list */
...
function drawNebulae(t){
  for(var i=0;i<nebulae.length;i++){
    var n=nebulae[i];
    var pulse=0.84+0.16*Math.sin(t*0.11+n.phase);   /* calmer: shallower, slower pulse */
...
/* Alive-hub list is read every frame by drawBackbone + drawNebulaTrails. Hub
   membership only shrinks (a node dies once and never revives, single write
   site) and the nodes array is fixed after init, so we build the list once and
   rebuild only on death (which nulls _hubsAlive). Replaces two per-frame
   nodes.filter() allocations + closures. */
var _hubsAlive=null;
function getAliveHubs(){
  if(_hubsAlive===null){
    _hubsAlive=[];
    for(var i=0;i<nodes.length;i++){ var n=nodes[i]; if(n.tier==='hub'&&!n.dead) _hubsAlive.push(n); }
  }
  return _hubsAlive;
}
...
function drawConns(){
  /* Active brightening between same-city districts when they're glowing —
     ...
     glow is dynamic so the set must be rebuilt each frame, but we fill a
     reused buffer with a plain loop instead of allocating a new array +
     closure via nodes.filter() every frame. */
  var act=_connScratch; act.length=0;
  for(var qi=0;qi<nodes.length;qi++){ var qn=nodes[qi];
    if(qn.glow>.05&&qn.tier==='district'&&!qn.dead&&qn.discovered) act.push(qn); }
  ctx.lineCap='round';
...
var fleet1,fleet2;
/* Central no-park zone: roughly the globe's on-screen footprint in canvas
   space. Ship and fleet targets avoid it so nothing parks where the globe
   forms and then gets shoved outward when the globe inflates on scroll.
   Computed from live CW so it stays correct across resizes. */
function _awayFromCentre(x,y){ var dx=x-CW/2, dy=y-CH/2, r=CW*0.17; return dx*dx+dy*dy > r*r; }
function pickFleetTarget(fl){var all=nodes.filter(function(n){return n.tier==='hub'&&!n.dead;});if(!all.length)return;var h=all.filter(function(n){return _awayFromCentre(n.x,n.y);});if(!h.length)h=all;var t=h[Math.random()*h.length|0];fl._target=t;fl.tx=t.x;fl.ty=t.y;fl.state='moving';}
...
/* LEFT batch — two warm craft in the left band (x .06-.46), drifting rightward,
   resetting before the centre globe so they read as a left-side convoy. */
var freighter={x:CW*.10,y:CH*.38,vx:.038,vy:.012,ang:0,trail:[],mT:30,size:2.2};
function updFreighter(dt){freighter.ang+=dt*.08;freighter.x+=freighter.vx*dt*60;freighter.y+=freighter.vy*dt*60;freighter.trail.push({x:freighter.x,y:freighter.y});if(freighter.trail.length>freighter.mT)freighter.trail.shift();if(freighter.x>CW*.46){freighter.x=CW*.06;freighter.y=CH*.26+Math.random()*CH*.24;freighter.vy=(Math.random()-.5)*.04;}}

/* Mirror freighter on the RIGHT band (x .60–.94), drifting leftward — balances the
   warm movement that otherwise only ever appears on the left. Stays clear of the
   centre globe (x>.60). */
var freighter2={x:CW*.92,y:CH*.40,vx:-.040,vy:.016,ang:0,trail:[],mT:30,size:2.0};
function updFreighter2(dt){freighter2.ang+=dt*.08;freighter2.x+=freighter2.vx*dt*60;freighter2.y+=freighter2.vy*dt*60;freighter2.trail.push({x:freighter2.x,y:freighter2.y});if(freighter2.trail.length>freighter2.mT)freighter2.trail.shift();if(freighter2.x<CW*.60){freighter2.x=CW*.94;freighter2.y=CH*.22+Math.random()*CH*.36;freighter2.vy=(Math.random()-.5)*.04;}}

/* Second right-band ship, lower and a touch slower — gives the right side two
   craft instead of one. Same right band (x .60-.94), drifts leftward, respawns
   on the lower-right so it never overlaps freighter2's path. */
var freighter3={x:CW*.86,y:CH*.66,vx:-.034,vy:-.012,ang:0,trail:[],mT:30,size:2.2};
function updFreighter3(dt){freighter3.ang+=dt*.08;freighter3.x+=freighter3.vx*dt*60;freighter3.y+=freighter3.vy*dt*60;freighter3.trail.push({x:freighter3.x,y:freighter3.y});if(freighter3.trail.length>freighter3.mT)freighter3.trail.shift();if(freighter3.x<CW*.60){freighter3.x=CW*.94;freighter3.y=CH*.54+Math.random()*CH*.34;freighter3.vy=(Math.random()-.5)*.04;}}

/* LEFT batch, second ship — lower-left, mirrors freighter3's role on the right. */
var freighterL2={x:CW*.06,y:CH*.72,vx:.034,vy:-.012,ang:0,trail:[],mT:30,size:2.0};
function updFreighterL2(dt){freighterL2.ang+=dt*.08;freighterL2.x+=freighterL2.vx*dt*60;freighterL2.y+=freighterL2.vy*dt*60;freighterL2.trail.push({x:freighterL2.x,y:freighterL2.y});if(freighterL2.trail.length>freighterL2.mT)freighterL2.trail.shift();if(freighterL2.x>CW*.46){freighterL2.x=CW*.06;freighterL2.y=CH*.52+Math.random()*CH*.30;freighterL2.vy=(Math.random()-.5)*.04;}}

/* LEFT batch, third ship — mid-left, fills the gap between the upper and lower two. */
var freighterL3={x:CW*.06,y:CH*.55,vx:.036,vy:.010,ang:0,trail:[],mT:30,size:2.1};
function updFreighterL3(dt){freighterL3.ang+=dt*.08;freighterL3.x+=freighterL3.vx*dt*60;freighterL3.y+=freighterL3.vy*dt*60;freighterL3.trail.push({x:freighterL3.x,y:freighterL3.y});if(freighterL3.trail.length>freighterL3.mT)freighterL3.trail.shift();if(freighterL3.x>CW*.46){freighterL3.x=CW*.06;freighterL3.y=CH*.40+Math.random()*CH*.24;freighterL3.vy=(Math.random()-.5)*.04;}}

/* RIGHT batch, third ship — upper-right, above freighter2/3. */
var freighterR3={x:CW*.94,y:CH*.28,vx:-.037,vy:.012,ang:0,trail:[],mT:30,size:2.1};
function updFreighterR3(dt){freighterR3.ang+=dt*.08;freighterR3.x+=freighterR3.vx*dt*60;freighterR3.y+=freighterR3.vy*dt*60;freighterR3.trail.push({x:freighterR3.x,y:freighterR3.y});if(freighterR3.trail.length>freighterR3.mT)freighterR3.trail.shift();if(freighterR3.x<CW*.60){freighterR3.x=CW*.94;freighterR3.y=CH*.18+Math.random()*CH*.28;freighterR3.vy=(Math.random()-.5)*.04;}}

var probe={x:-10,y:CH*.38,vx:.012,vy:.003,ang:0,trail:[],mT:20,active:true};
function updProbe(dt){if(!probe.active)return;probe.ang+=dt*.04;probe.x+=probe.vx*dt*60;probe.y+=probe.vy*dt*60;probe.trail.push({x:probe.x,y:probe.y});if(probe.trail.length>probe.mT)probe.trail.shift();if(probe.x>CW+20){probe.active=false;setTimeout(function(){probe.x=-10;probe.y=CH*.2+Math.random()*CH*.5;probe.active=true;},60000+Math.random()*60000);}}

var robber={x:CW*.30,y:CH*.24,tx:CW*.28,ty:CH*.22,vx:0,vy:0,spd:.52,trail:[],mT:70,ang:0,pause:0};
var cop={x:CW*.30,y:CH*.27,tx:0,ty:0,vx:0,vy:0,spd:.46,trail:[],mT:60,ang:0};
var esc1={x:CW*.31,y:CH*.25,vx:0,vy:0,spd:.32,trail:[],mT:45,ang:0};
var esc2={x:CW*.29,y:CH*.25,vx:0,vy:0,spd:.18,trail:[],mT:45,ang:0};
var chLasers=[];
function newWaypoint(){
  ...
  if(targets.length>0){
    ...
    /* No off-centre hub on screen: aim at a point on a ring outside the central
       globe zone, so the chase circles the globe rather than diving through it. */
    var a=Math.random()*6.2832, rr=CW*(0.20+Math.random()*0.08);
    robber.tx=CW/2+Math.cos(a)*rr;
    robber.ty=CH/2+Math.sin(a)*rr;
  }
}
...
function drawChaseShip(ship,size,warm,threat,col){
  if(!fin(ship.x,ship.y))return;
  var base = col || (threat?[255,72,58]:(warm?[240,220,180]:[228,234,246]));
  var R=size*3.8*CRAFT.halo;
  var h=rg(ship.x,ship.y,0,ship.x,ship.y,R);
  var auraCol='rgba('+base[0]+','+base[1]+','+base[2]+','+cA(threat?0.78:0.72).toFixed(3)+')';
  if(h){h.addColorStop(0,auraCol);h.addColorStop(1,'rgba(80,100,140,0)');ctx.beginPath();ctx.arc(ship.x,ship.y,R,0,Math.PI*2);ctx.fillStyle=h;ctx.fill();}
  ...
  ctx.fillStyle='rgba('+base[0]+','+base[1]+','+base[2]+','+cA(threat?1.0:0.98).toFixed(3)+')';ctx.fill();ctx.restore();
}
function drawChase(){
  ...
  ctx.globalAlpha=robberDamageAlpha();drawChaseShip(robber,8.5,false,true);ctx.globalAlpha=1;drawChaseShip(cop,7.5,false);drawChaseShip(esc1,4.4,false,true);drawChaseShip(esc2,4.4,false,true);
}
function drawFleet(fl){var FCOLS=[[228,234,246],[228,234,246],[228,234,246]];for(var i=0;i<fl.ships.length;i++){var s=fl.ships[i];var col=FCOLS[i%FCOLS.length];for(var j=1;j<s.trail.length;j++){var f=j/s.trail.length,p0=s.trail[j-1];if(!fin(p0.x,p0.y,s.trail[j].x,s.trail[j].y))continue;ctx.beginPath();ctx.moveTo(p0.x,p0.y);ctx.lineTo(s.trail[j].x,s.trail[j].y);ctx.strokeStyle='rgba('+col[0]+','+col[1]+','+col[2]+','+(f*f*.2).toFixed(3)+')';ctx.lineWidth=f*.62;ctx.stroke();}drawChaseShip(s,5.2,false,false,col);}}
function drawPatrol(p){var s=p.ship;if(!s)return;for(var j=1;j<s.trail.length;j++){var f=j/s.trail.length,p0=s.trail[j-1];if(!fin(p0.x,p0.y,s.trail[j].x,s.trail[j].y))continue;ctx.beginPath();ctx.moveTo(p0.x,p0.y);ctx.lineTo(s.trail[j].x,s.trail[j].y);ctx.strokeStyle='rgba(180,200,230,'+(f*f*.12).toFixed(3)+')';ctx.lineWidth=f*.45;ctx.stroke();}drawChaseShip(s,2.8,false);}
function drawSlowShip(ship,sz,warm){if(!fin(ship.x,ship.y))return;for(var j=1;j<ship.trail.length;j++){var f=j/ship.trail.length,p0=ship.trail[j-1];if(!fin(p0.x,p0.y,ship.trail[j].x,ship.trail[j].y))continue;ctx.beginPath();ctx.moveTo(p0.x,p0.y);ctx.lineTo(ship.trail[j].x,ship.trail[j].y);ctx.strokeStyle='rgba(180,200,230,'+(f*f*.09).toFixed(3)+')';ctx.lineWidth=f*.4;ctx.stroke();}drawChaseShip(ship,sz,warm);}

var wan={x:CW*.74,y:CH*.26,tx:CW*.70,ty:CH*.22,vx:0,vy:0,spd:.32,trail:[],mT:72,ang:0};
var orb={angle:0,spd:.022,rX:0,rY:0,cx:CW*.24,cy:CH*.26,tilt:0,tiltSpd:.0012,trail:[],mT:50,x:CW*.24,y:CH*.26};
var sct={x:-20,y:CH*.4,tx:CW+20,ty:CH*.6,vx:0,vy:0,spd:.18,trail:[],mT:58,ang:0,dT:0};
var deadSat={x:CW*.26,y:CH*.72,vx:.022,vy:.008,ang:0,angSpd:.025,trail:[],mT:40,alpha:.25};
var relaySat={x:CW*.50,y:CH*.43,ang:0,pulseT:0,pulseNext:6};
var geoSat={x:CW*.58,y:CH*.50,ang:0};
var reticles=[];

function initOrb(){var h=nodes.filter(function(n){return n.tier==='hub';});if(h.length){orb.cx=h[0].x;orb.cy=h[0].y;}orb.rX=45+Math.random()*20;orb.rY=22+Math.random()*12;orb.tilt=Math.random()*.5;}
...
function updWan(dt){
  wan.ang+=dt*.36;
  /* Reynolds wander: the target drifts smoothly along a slowly-turning angle
     projected ahead of the craft, so it meanders organically instead of hopping
     between points and pausing. Same velocity model as before, so speed/feel
     stay calibrated — only HOW the target is chosen changed. */
  wan.wAng=(wan.wAng===undefined?Math.random()*6.2832:wan.wAng)+(Math.random()-0.5)*0.9*dt;
  var sp=Math.sqrt(wan.vx*wan.vx+wan.vy*wan.vy);
  var hx=sp>0.001?wan.vx/sp:Math.cos(wan.wAng), hy=sp>0.001?wan.vy/sp:Math.sin(wan.wAng);
  wan.tx=wan.x+hx*90+Math.cos(wan.wAng)*70;
  wan.ty=wan.y+hy*90+Math.sin(wan.wAng)*70;
  // soft containment within the comfortable on-screen band
  if(wan.tx<CW*0.15)wan.tx=CW*0.15; else if(wan.tx>CW*0.85)wan.tx=CW*0.85;
  if(wan.ty<CH*0.13)wan.ty=CH*0.13; else if(wan.ty>CH*0.87)wan.ty=CH*0.87;
  var dx=wan.tx-wan.x,dy=wan.ty-wan.y,d=Math.sqrt(dx*dx+dy*dy)||0.001;
  var s=Math.min(.85,d*.008)*60;
  wan.vx=wan.vx*.88+(dx/d)*s*.12;wan.vy=wan.vy*.88+(dy/d)*s*.12;
  wan.x+=wan.vx*dt;wan.y+=wan.vy*dt;
  wan.trail.push({x:wan.x,y:wan.y});if(wan.trail.length>wan.mT)wan.trail.shift();
}
...
var bigAst={x:-120,y:CH*.3,vx:.035,vy:.010,ang:0,trail:[],mT:50,debris:[],active:true};
...
function updBigAst(dt){bigAst.ang+=dt*.06;bigAst.x+=bigAst.vx*dt*60;bigAst.y+=bigAst.vy*dt*60;bigAst.trail.push({x:bigAst.x,y:bigAst.y});if(bigAst.trail.length>bigAst.mT)bigAst.trail.shift();if(bigAst.x>CW+150){bigAst.x=-120;bigAst.y=CH*.15+Math.random()*CH*.7;bigAst.vy=(Math.random()-.5)*.03;}}
...
/* ── Globe interaction: swing-by + depth ───────────────────────────────
   The globe module publishes its screen-space disk as window.SITE.__globeScreen
   ({x,y,r} in CSS px, on:true while visible). The 2D layer sits at (OX,OY)
   at 1:1 px scale, so a screen point maps to canvas space by subtracting
   OX/OY.

   Two effects combine to make ships feel like they share 3D space with the
   globe:
   1. Swing-by — near the globe, a craft's heading is bent toward the tangent
      (the way it's already going round) plus a little outward, speed kept
      constant, so it curves past in a graceful slingshot arc instead of a
      straight line. No hard wall, so committed paths can still cross.
   2. Depth — a craft drawn while inside the disk is faded toward the centre
      (and is already behind the globe canvas in z-order), so it reads as
      passing BEHIND the sphere and re-emerging, rather than slicing the
      wireframe. Applied via globalAlpha around each craft's draw. */
var GLOBE_AVOID_PAD   = 10;    // px clearance folded into the swing radius
var GLOBE_SWING_RANGE = 1.75;  // influence radius = (r+pad) * this
var GLOBE_SWING_BEND  = 0.34;  // how hard the path curves near the globe
var GLOBE_BEHIND_MIN  = 0.10;  // craft alpha at the globe's centre (deepest "behind")

function _swingXY(o, XK,YK,VXK,VYK, gx,gy,R){
  var dx=o[XK]-gx, dy=o[YK]-gy, d=Math.sqrt(dx*dx+dy*dy);
  var Rinf=R*GLOBE_SWING_RANGE;
  if(d>=Rinf||d<1e-4) return;
  var nx=dx/d, ny=dy/d;                          // outward radial
  var prox=(Rinf-d)/(Rinf-R); if(prox<0)prox=0; if(prox>1.4)prox=1.4;
  var vx=o[VXK], vy=o[VYK], sp=Math.sqrt(vx*vx+vy*vy);
  if(sp<1e-4) return;
  var tx=-ny, ty=nx;                             // tangent
  if(vx*tx+vy*ty<0){ tx=-tx; ty=-ty; }           // pick the way it's already orbiting
  var bend=prox*prox*GLOBE_SWING_BEND;
  var nvx=vx + (tx*1.15 + nx*0.55)*sp*bend;       // curve toward tangent + slight outward
  var nvy=vy + (ty*1.15 + ny*0.55)*sp*bend;
  var nsp=Math.sqrt(nvx*nvx+nvy*nvy)||1e-4;
  o[VXK]=nvx/nsp*sp;                              // renormalise: a slingshot bends, doesn't speed up
  o[VYK]=nvy/nsp*sp;
}
function steerBodiesAroundGlobe(){
  var g=window.SITE.__globeScreen;
  if(!g||!g.on) return;
  var gx=g.x-OX, gy=g.y-OY, R=g.r+GLOBE_AVOID_PAD;
  // Fleets: steer the formation CENTRE (the ships follow it), not each ship.
  if(fleet1) _swingXY(fleet1,'cx','cy','vx','vy',gx,gy,R);
  if(fleet2) _swingXY(fleet2,'cx','cy','vx','vy',gx,gy,R);
  // Chase group + free craft: steer their own velocity.
  _swingXY(robber,'x','y','vx','vy',gx,gy,R);
  _swingXY(cop,'x','y','vx','vy',gx,gy,R);
  _swingXY(esc1,'x','y','vx','vy',gx,gy,R);
  _swingXY(esc2,'x','y','vx','vy',gx,gy,R);
  _swingXY(freighter,'x','y','vx','vy',gx,gy,R);
  _swingXY(freighter2,'x','y','vx','vy',gx,gy,R);
  _swingXY(freighter3,'x','y','vx','vy',gx,gy,R);
  _swingXY(freighterL2,'x','y','vx','vy',gx,gy,R);
  _swingXY(freighterL3,'x','y','vx','vy',gx,gy,R);
  _swingXY(freighterR3,'x','y','vx','vy',gx,gy,R);
  if(probe.active) _swingXY(probe,'x','y','vx','vy',gx,gy,R);
  _swingXY(wan,'x','y','vx','vy',gx,gy,R);
  _swingXY(sct,'x','y','vx','vy',gx,gy,R);
  _swingXY(deadSat,'x','y','vx','vy',gx,gy,R);
  // patrols (parametric line) + orb (own orbit) rely on the depth fade only.
}

/* Depth fade: 1 outside the globe disk, dropping toward GLOBE_BEHIND_MIN at the
   centre, so a craft passing over the globe dims as if going behind it. */
function _depthAlpha(x,y){
  var g=window.SITE.__globeScreen; if(!g||!g.on) return 1;
  var gx=g.x-OX, gy=g.y-OY, R=g.r;
  var dx=x-gx, dy=y-gy, d2=dx*dx+dy*dy;
  if(d2>=R*R) return 1;
  var tt=Math.sqrt(d2)/R;                          // 0 centre .. 1 edge
  return GLOBE_BEHIND_MIN + (1-GLOBE_BEHIND_MIN)*tt*tt;
}
var _fxPrevA=1;
function _fxBegin(x,y){ var a=_depthAlpha(x,y); _fxPrevA=ctx.globalAlpha; if(a<0.999) ctx.globalAlpha=_fxPrevA*a; }
function _fxEnd(){ ctx.globalAlpha=_fxPrevA; }

/* Ships + the four moving satellites. Called inside the gate at its original
   z-order for the normal phase, and again after the gate during the globe
   phase. Each craft's draw is wrapped in the depth fade so it occludes behind
   the globe. */
function drawShipsAndMovingSats(){
  if(!RM){
    _fxBegin(fleet1.cx,fleet1.cy); drawFleet(fleet1); _fxEnd();
    _fxBegin(fleet2.cx,fleet2.cy); drawFleet(fleet2); _fxEnd();
    if(patrol1.ship){ _fxBegin(patrol1.ship.x,patrol1.ship.y); drawPatrol(patrol1); _fxEnd(); }
    if(patrol2.ship){ _fxBegin(patrol2.ship.x,patrol2.ship.y); drawPatrol(patrol2); _fxEnd(); }
    _fxBegin(freighter.x,freighter.y); drawSlowShip(freighter,freighter.size,true); _fxEnd();
    _fxBegin(freighter2.x,freighter2.y); drawSlowShip(freighter2,freighter2.size,true); _fxEnd();
    _fxBegin(freighter3.x,freighter3.y); drawSlowShip(freighter3,freighter3.size,true); _fxEnd();
    _fxBegin(freighterL2.x,freighterL2.y); drawSlowShip(freighterL2,freighterL2.size,true); _fxEnd();
    _fxBegin(freighterL3.x,freighterL3.y); drawSlowShip(freighterL3,freighterL3.size,true); _fxEnd();
    _fxBegin(freighterR3.x,freighterR3.y); drawSlowShip(freighterR3,freighterR3.size,true); _fxEnd();
    if(probe.active){ _fxBegin(probe.x,probe.y); drawSlowShip(probe,.8,false); _fxEnd(); }
  }
  _fxBegin(orb.x,orb.y); drawOrb(); _fxEnd();
  _fxBegin(wan.x,wan.y); drawWan(); _fxEnd();
  _fxBegin(sct.x,sct.y); drawSct(); _fxEnd();
  _fxBegin(deadSat.x,deadSat.y); drawDeadSat(); _fxEnd();
}

/* ── MAIN LOOP ── */
var RM = REDUCED_MOTION;
/* The nebula (WebGL shader + 2D glow/starfield) and the interactive hub
   constellation are an intro flourish, not a permanent backdrop — after a
   short window they stop entirely and the WebGL canvas is hidden, leaving
   the plain solid background. Ships and satellites are unaffected and keep
...
function loop(ts){
  ...
  try{
    ...
    _budget.step(rdt);
    /* Globe handoff: while the scroll-arc globe owns the screen (its opaque
       canvas fully covers the background) skip the nebula WebGL render and the
       entire 2D scene draw — they're invisible underneath it. Updates still run
       below so nothing freezes or snaps when you scroll back up. clearRect stays
       (cheap on the 1.65x surface) so no stale frame flashes at the handoff. */
    var _globeOwns = (window.SITE.__globeOwnsScreen === true);
    /* Scroll choreography removed: the CRT, globe and footer are normal-flow
       sections now. The background stays at constant brightness (no scroll-driven
       dimming), and the footer's reveal + render gate are handled by an
       IntersectionObserver near the footer script rather than by scroll progress. */
    window.SITE.__netAlpha = 0.5;
    var _calm = 0, _busy = 1, _netA = 1;
    window.SITE.__footerCalm = 0;
    if(renderer && !_bgIntroOver) renderer.render();   // nebula renders in every phase (globe + tour sit over it), until the intro window ends
    ctx.clearRect(0,0,CW,CH);
    ctx.lineCap='round';
    /* Nebula + starfield draw in every phase now (incl. the globe form and the
       project tour) so the globe and cards sit over a live nebula, not black —
       but only within the intro window; after that this whole layer stops.
       The crosshair grid stays gated — it reads as "network", not atmosphere. */
    if(!_bgIntroOver){
    drawNebulae(t);drawStarsParallax(t);
    if(!_globeOwns && _netA>0.01){
      ctx.save(); ctx.globalAlpha=_netA;
      drawCrosshairGrid();
      ctx.restore();
    }
    }
    // Footer calm splits the scene in two:
    //   KEEP (always full opacity, persists into the footer): nebula, stars, ships,
    //         satellites, asteroids, shooting stars, meteors — the calm "space traffic".
    //   FADE (gated by _netA -> 0 at the footer): the network map — crosshair grid,
    //         node labels, constellation/connection lines, traceroutes, scan rings,
    //         wormhole, intrusions, glitch, data storm, backbone, mem-lines.
    // _netA is the network-map alpha; ships never read it, so they stay.
    // _netA (network-map alpha) computed above with _calm; fully faded by calm 0.42.
    updReveal(dt);
    if(!RM) updGlitch(dt);
    ...
    if(!_bgIntroOver){ for(var i=0;i<nlen;i++) nodes[i].update(dt); }
    ...
    if(!RM){
      ...
      updFreighter(dt);updFreighterL2(dt);updFreighterL3(dt);updFreighter2(dt);updFreighter3(dt);updFreighterR3(dt);updProbe(dt);
    ...
    updAfterGlows(dt);
    updOverloads(dt);
    /* keep ships + the moving satellites clear of the globe's screen disk */
    steerBodiesAroundGlobe();
    if(!_globeOwns){
    // ---- NETWORK MAP (fades out at the footer via _netA) ----
    if(_netA>0.01){
    ctx.save(); ctx.globalAlpha *= _netA;
    ...
    /* Beat 2 is space only: nebula, ships, satellites. The node web and the
       traceroutes belong to the intro; the globe's own formation brings the
       dots back for beat 2's second half. */
    /* renamed: v9 already has a _netA in this scope for the footer fade */
    var _netGate=(window.SITE.__netAlpha===undefined)?1:window.SITE.__netAlpha;
    if(_netGate>0.004 && !_bgIntroOver){
    ctx.save(); ctx.globalAlpha*=_netGate;
    drawGalaxy();
    if(scrollY < (window.innerHeight||800)*1.25) drawConstellationLines();   /* constellations: intro only */
    if(_budget.tier < 3) drawCorona(t);
    if(_budget.tier < 2) drawNebulaTrails();
    drawMemLines();drawBackbone();
    for(var i=0;i<nlen;i++) nodes[i].drawTrail();
    drawPathGlows();
    drawConns();drawPulses();drawDS();drawTraceroutes();drawScanRings();
    for(var i=0;i<nlen;i++) nodes[i].drawRings();
    for(var i=0;i<nlen;i++) nodes[i].drawHB(t);
    for(var i=0;i<nlen;i++) nodes[i].draw(t);
    ...
    // ---- SPACE TRAFFIC (always full opacity, stays in the footer) ----
    drawShipsAndMovingSats();
    drawRelaySat();drawGeoSat();
    if(!RM){
      drawAst();drawBigAst();drawSS();
      drawMeteorShower();
    }
    drawAfterGlows();
    // grain + glitch are network-map texture -> fade them with _netA too
    if(!RM && _netA>0.01){
      ctx.save(); ctx.globalAlpha *= _netA;
      drawGrain(t);
    ...
    } /* end !_globeOwns draw gate */

    /* Globe / project-tour phase: the dense network is gated off above, but the
       ships + moving satellites keep flying over the globe — steered clear of it
       by steerBodiesAroundGlobe(). Drawn here (not inside the gate) so they ride
       on top of the live nebula during the globe phase. */
    if(_globeOwns) drawShipsAndMovingSats();

    // The red ship (the chase) stays present through the whole experience — drawn even
    // over the globe/project area — but NOT during the boot intro or the CRT hero at the
    // top. __arcProgress > ~0.06 means we've scrolled off the CRT screen.
    if(!RM && revealDone){ _fxBegin(robber.x,robber.y); drawChase(); _fxEnd(); }
    ctx.globalAlpha = 1;   // always reset so the calm fade never leaks into other layers
  }catch(e){
...
function _doResize(){
  ...
  if(renderer) renderer.setSize(SW, SH);   // renderer is null when WebGL init failed
  ...
  _bakeGrain();
  window.SITE.__bfTop = undefined;   // force _calm to re-measure the beat-footer offset next frame
}
